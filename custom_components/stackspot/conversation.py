...
from .const import DOMAIN, CONF_AGENT, AGENTS_KEY
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities) -> None:
    ...
    if stackspot_agent_instance:
        async_add_entities([
            StackSpotConversationEntity(hass, entry_id, agent_id, stackspot_agent_instance)
        ], True)
...
class StackSpotConversationEntity(ConversationEntity):
    ...
    def __init__(
            self, hass: HomeAssistant, config_entry_id: str, agent_id: str, agent_instance: StackSpotAgent
    ) -> None:
        ...
        self._attr_device_info = DeviceInfo(
            ...
            name=f"StackSpot AI ({agent_id})",
            manufacturer="StackSpot Integration",
        )
    ...
    async def async_process(self, user_input: ConversationInput) -> ConversationResult:
        ...
        _LOGGER.debug(f"Conversation entity received input for agent {self._agent_id}: '{user_input.text}'")
